...
class BenchContorller:
    ...
    def get_sorted_first_coordinate(self, timeout=1.0, recognition_scheme="start1"):
        """
        从队列中获取所有坐标数据，根据识别方案进行排序，
        然后返回排在第一个的坐标数据
        
        Args:
            timeout: 等待超时时间（秒）
            recognition_scheme: 识别方案，影响排序规则
        ...
        try:
            ...
            if recognition_scheme == "start1":
                # start1方案：先按Y轴从大到小分组，组内X轴从大到小
                sorted_coordinates = sorted(all_coordinates, 
                    key=lambda coord: (-get_y_group(coord['y']), -coord['x']))
                print(f"使用start1排序规则：先按Y轴从大到小分组（3mm容忍度），组内X轴从大到小")
            else:
                # 其他方案：先按Y轴从大到小分组，组内X轴从小到大
                sorted_coordinates = sorted(all_coordinates, 
                    key=lambda coord: (-get_y_group(coord['y']), coord['x']))
                print(f"使用默认排序规则：先按Y轴从大到小分组（3mm容忍度），组内X轴从小到大")
    ...
    def put_back(self, approach_height: float = 700.0, pick_height: float = 446.0, timeout: float = 10.0, recognition_scheme: str = "start1") -> Dict[str, Any]:
        ...
        try:
            ...
            # 步骤3: 打开夹爪进行放置
            print("步骤3: 打开夹爪进行放置")
            gripper_result = self.gripper_control_move(position=33, timeout=5)
            if not gripper_result["success"]:
                return {
                    "success": False,
                    "message": f"打开夹爪失败或超时: {gripper_result.get('message', '未知错误')}",
